  ...
  const ins = await pool.query(
    `INSERT INTO drip_enrollment
       (sequence_id, lead_ref, conversation_id, source, vertical, channel, phone_e164,
        category_raw, category_key, time_zone, step, t0_at, next_due_at, max_messages, expires_at, first_name, city)
    ...
    [
      ...
      Number(seq.max_messages), expiresAt.toISOString(), lead.firstName || null, lead.city || null,
    ],
  );
...
export async function getDue(pool, now = new Date(), limit = 50) {
  const r = await pool.query(
    `SELECT id, sequence_id, lead_ref, conversation_id, source, vertical, phone_e164,
            category_key, category_raw, city, first_name, time_zone, step, t0_at, attempts, max_messages, expires_at, status
       FROM drip_enrollment
