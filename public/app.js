...
function addServiceRowFromSearch(el) {
  // Get the currently visible option (the last one, or the only one if just one)
  if (!state.options || !state.options.length) {
    alert('No options available. Create an estimate first.');
    return;
  }
  
  // Add to the first option by default (could be enhanced to let user choose)
  const optionIndex = 0;
  ...
  const newLine = {
    ...
    frequency: 'single',
    unitPrice: Number(el.dataset.itemPrice) / 100, // Convert from cents to dollars
    pricingMode: 'calculated',
    flatAmount: 0,
...
function renderEstimateSpreadsheetEditor(options, { lockTaxable = false, allowOptionNameEdit = true } = {}) {
  ...
  const optionBlocks = options.map((opt, optionIndex) => {
    const optionLabel = allowOptionNameEdit
      ? `<input type="text" class="sr-input sheet-opt-name" data-ed-option-name="${optionIndex}" value="${esc(opt.name || `Option ${optionIndex + 1}`)}" />`
      : `<strong>${esc(opt.name || `Option ${optionIndex + 1}`)}</strong>`;

    const rows = (opt.lineItems || []).map((li, lineIndex) => {
      ...
      return `
        <tr class="${recurringClass}" data-ed-row="${optionIndex}:${lineIndex}">
          <td><input type="text" class="sr-input" data-ed-field="name" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${esc(li.name || '')}" /></td>
          <td><input type="text" class="sr-input" data-ed-field="description" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${esc(li.description || '')}" /></td>
          <td>
            <div class="sheet-unit-group">
              <select class="sr-input" data-ed-field="unitOfMeasure" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}">
                ${renderUnitOptions(li.unitOfMeasure || '', availableUnits)}
              </select>
              <div class="sheet-unit-qty-label" data-ed-qty-label="${optionIndex}:${lineIndex}">${esc(formatQtyContextLabel(li.unitOfMeasure))}</div>
              <input type="number" step="any" class="sr-input sr-number ${qtyInvalid ? 'sheet-invalid' : ''}" data-ed-field="quantity" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${qty}" />
            </div>
          </td>
          <td>
            <select class="sr-input" data-ed-field="frequency" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}">
              ${renderFrequencyOptions(li.frequency)}
            </select>
          </td>
          <td><input type="number" step="0.01" class="sr-input sr-number ${priceInvalid ? 'sheet-invalid' : ''}" data-ed-field="unitPrice" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${unitPrice}" /></td>
          <td>
            <div class="sheet-pricing-group">
              <select class="sr-input" data-ed-field="pricingMode" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}">
                ${renderPricingModeOptions(pricingMode)}
              </select>
              <input type="number" step="0.01" class="sr-input sr-number ${flatInvalid ? 'sheet-invalid' : ''}" data-ed-field="flatAmount" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${toNumber(li.flatAmount, 0)}"${isFlat ? '' : ' disabled'} />
              <div class="sheet-pricing-hint" data-ed-pricing-hint="${optionIndex}:${lineIndex}">${isFlat ? 'Flat amount override' : 'Auto: Qty x Unit Price'}</div>
            </div>
          </td>
          <td><input type="text" class="sr-input ${kindInvalid ? 'sheet-invalid' : ''}" data-ed-field="kind" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${esc(li.kind || 'labor')}" /></td>
          <td><input type="checkbox" data-ed-field="taxable" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}"${taxAttr}></td>
          <td><input type="text" class="sr-input" data-ed-field="notes" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}" value="${esc(li.notes || '')}" /></td>
          <td data-ed-amount="${optionIndex}:${lineIndex}">${money(amount)}${isFlat ? '<span class="sheet-badge flat">Flat</span>' : ''}</td>
          <td class="sheet-actions-col">
            <div class="sheet-badges" data-ed-badges="${optionIndex}:${lineIndex}">${renderFrequencyBadge(li.frequency)}${renderLineValidationBadges(issues)}</div>
            <button type="button" class="sheet-mini-btn" data-ed-action="duplicate-row" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}">Duplicate</button>
            <button type="button" class="sheet-mini-btn danger" data-ed-action="delete-row" data-ed-option="${optionIndex}" data-ed-line="${lineIndex}">Delete</button>
          </td>
        </tr>`;
    }).join('');
      ...
      <div class="sheet-option-block">
        <div class="sheet-option-header">
          ...
          <div class="sheet-option-tools">
            <button type="button" class="sheet-mini-btn" data-ed-action="add-row" data-ed-option="${optionIndex}">+ Add Row</button>
            ${canRemoveOption ? `<button type="button" class="sheet-mini-btn danger" data-ed-action="remove-option" data-ed-option="${optionIndex}">Remove Option</button>` : ''}
            <span class="hint">Subtotal: <strong data-ed-option-total="${optionIndex}">${money(opt.total || 0)}</strong></span>
          </div>
        </div>
  ...
  return `
    <div class="sheet-toolbar">
      <div class="hint">Spreadsheet editor: changes are reflected in dry run and create.</div>
